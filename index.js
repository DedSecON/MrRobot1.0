...
const client = new WAConnection()
   ...
   console.log(color('♥','white'),color('♥','red'),color('♥','white'),color('SQAN CODENYA!!','white'),color('YOU','red'),color('TUBE','white'),color('DappaUhuy','yellow'))
})
...
// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*BOT | DappaUhuy√ HARUS JADI ADMINâ—*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
...
break
		...
		case 'resepmasakan':  
			...
			if (!isGroupAdmins) return reply(ind.admin())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group BOT | DappaUhuy :\n\n\`\`\``
					no = 0
	...
	case 'hobby':
		...
		case 'next':
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam *${prefix}menu BOT | DappaUhuy√*`)
